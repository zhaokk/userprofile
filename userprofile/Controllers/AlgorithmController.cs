...
namespace userprofile.Controllers
{
    public class AlgorithmController : Controller
    {
        ...
        bool checkTabu(int oID, int rID)
        { //check if this is tabu
            return false;
        }

        bool checkAvailable(int oID, int rID)
        { //check if this ref is available to referee this offer (mostly for conflicts timewise)
            return true;
        }

        void setPairUnavailable(int oID, int rID)
        { //make combination of Offer/Ref unavailable
        ...
        void updateAvailability(bool addRemove, int oID, int rID)
        { //Update availability of referee
            if (addRemove)
            { //if assigning a ref to an offer
                setOfferUnavailable(oID);
                if (dReferees[rID].canAssign == dReferees[rID].assignedTo.Count())
                { //if assigned to max amount of offers that ref can have
        ...
        void setRefereeUnavailable(int rID)
        { //Referee has been assigned to canAssign offers
        ...
        { //Offer has been assigned a referee
            LinkedList<int> nuke = new LinkedList<int>();
        ...
        {
            foreach (var i in qualificationRefStorage)
            { //make all qualificationRefStorage[qID] have all referee id's with qualification
                foreach (var j in db.REFEREEs)
                {
                    foreach (var k in j.QUALIFICATIONS)
                    {
                        if (i.Key == k.qID)
                        { //check level here later on
        ...
        { // get a random available Ref  TODO: MAKE IT PICK ONE WITH TABU IF NONE AVAILABLE
            Random rand = new Random();
        ...
        private void initChecks()
        {
            ...
            {
                foreach (var i in nuke)
                { //remove offers that have been finished
                    removeOffer(i);
                }
                nuke.Clear();
                foreach (var i in dReferees)
                {
                    if (i.Value.available.Count <= i.Value.canAssign)
                    { //if the ref is available to referee all games the can be assigned
            ...
            Random rand = new Random();
            foreach (var i in UniqueRandomValues(dOffers))
            { //randomise offers 
        ...
        public ActionResult showTestResult(){
